...
- AC-ING-002: Given repository payloads that are arrays or single objects, when parsed, then both forms produce normalized catalog records.
...
- AC-ING-011: Given boolean, zero, negative, malformed numeric, or unknown-game IDs, when normalized, then they are rejected with a reason rather than coerced.
...
- AC-ING-013: Given a completed run, then `discovered_in_scope = indexed + unavailable + malformed + unsupported + excluded`.
